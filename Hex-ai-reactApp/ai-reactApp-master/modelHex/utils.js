const getNumberOfPathsOfMinimumLength = require('./bfs');

export function seeVirtualBoard(board, move, playerId) {
    let row = move[0]
    let col = move[1]
    if (board[row][col] === 0) {
...
export function possibleMoves(board) {
    let hexes2 = getNumberOfPathsOfMinimumLength(board, '2', '1')[1];
    let hexes1 = getNumberOfPathsOfMinimumLength(board, '1', '2')[1];
    let result = []
    console.log(hexes1)
    for (let i = 0; i<hexes1.length;i++){
        for (let j = 0; j<i.length;j++){
            console.log(hexes1[i][j])
        }
...
export function gameIsOver(tablero) {
    let board = tablero;
